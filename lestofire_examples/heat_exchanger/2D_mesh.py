import pygmsh as pg
from heat_exchanger_nls import (
    height,
    width,
    dist_center,
    inlet_width,
...
def main():
    ...
    mouth_inlet1 = geom.add_rectangle(
        [-inlet_depth, ymin1, 0.0], inlet_depth, inlet_width
    )
    mouth_inlet2 = geom.add_rectangle(
        [-inlet_depth, ymin2, 0.0], inlet_depth, inlet_width
    )

    mouth_outlet1 = geom.add_rectangle(
        [width, ymin1, 0.0], inlet_depth, inlet_width
    )
    mouth_outlet2 = geom.add_rectangle(
        [width, ymin2, 0.0], inlet_depth, inlet_width
    )
    ...
    _ = geom.boolean_fragments(
        [main_rect], [mouth_inlet1, mouth_inlet2, mouth_outlet1, mouth_outlet2]
    )

    geom.add_raw_code(
        """vb1[] = Boundary{{Surface{{ {0} }};}};
