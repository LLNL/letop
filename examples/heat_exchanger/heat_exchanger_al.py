...
from lestofire import (
    LevelSetLagrangian,
    AugmentedLagrangianOptimization,
...
def main():
    ...
    phi_expr = (
        ...
        + line_sep
        + (y > line_sep + 0.2)
        * (-2.0 * sin((y - line_sep - 0.2) * pi / 0.5))
        * sin(x * pi / width)
        - (y < line_sep)
        * (0.5 * sin((y + line_sep / 3.0) * pi / (2.0 * line_sep / 3.0)))
        * sin(x * pi * 2.0 / width)
    )
    ...
    solver_stokes2 = LinearVariationalSolver(
        problem, solver_parameters=stokes_parameters
    )
