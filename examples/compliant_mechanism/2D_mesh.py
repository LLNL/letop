...
def main():
    ...
    spring_box = geom.add_rectangle(
        [width - width_spring, height / 2.0 - height_spring / 2.0, 0.0],
        width_spring,
        height_spring,
    )
    load_box = geom.add_rectangle(
        [0.0, height / 2.0 - height_load / 2.0, 0.0], width_load, height_load
    )

    geom.add_physical(spring_box, 1)
    geom.add_physical(load_box, 2)

    inverter = geom.boolean_fragments([main_rect], [spring_box, load_box])
    ...
    geom.add_raw_code(
        """vb1[] = Boundary{{Surface{{ {0} }};}};
                        ...
                        """.format(
            spring_box.id, load_box.id, inverter.id
        )
    )
