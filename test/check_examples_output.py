...
test_collection = [
    TestExample(folder="heat_exchanger", n_iter=10, cost_func_value=-0.9687),
    TestExample(folder="cantilever", n_iter=10, cost_func_value=124400),
]
...
def run_check(output_file, folder):
    ...
    with open(output_file, "r") as read_test:
        all_iters = re.findall(
            "([0-9]*)\. J=(-?[0-9]\d*)(\.\d+)?(e-?\+?\d*)?", read_test.read()
        )
